<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if (empty($posted_token) || !hash_equals($_SESSION['csrf_token'], $posted_token)) {
            ...
            $message = "<p style='color:red;'>CSRF token falta da edo ez da egokia.</p>";
        } else {
            ...
            if ($izena !== '' && $jatorria !== '' && $kolorea !== '' && $denbora !== '') {
                ...
                if ($stmt) {
                    ...
                    $message = "<p style='color:red;'>❌ Errore bat gertatu da prestatzean: " . htmlspecialchars($conn->error) . "</p>";
                }
            } else {
                $message = "<p style='color:red;'>❌ Datu guztiak bete behar dira.</p>";
            }
        }
    }
    ?>
